...
class Session
{
    ...
    public function session_write($id, $data) 
    {
        ...
        $db->query_free("REPLACE INTO `session` SET `session_id` = '".$id."', `sess_date` = '".$this->CurrentTime."', `session_data` = '".$data."'");
        if($db->table_query)
        {
            return true;
        }
    ...
    public function session_clean($maxlifetime) 
    {
    	$db = new db;
       	$db->db_connect();
        $db->query_free("DELETE FROM `session` WHERE ((UNIX_TIMESTAMP(`sess_date`) + ".$maxlifetime.") < ".$maxlifetime.")");
        if($result){
            return true;
        }else{
            return false;
